...
| 약어 | 원칙 이름                             | 의미 요약                 |
| ---- | ------------------------------------- | ------------------------- |
| S    | SRP (Single Responsibility Principle) | 단일 책임 원칙이다.       |
| O    | OCP (Open/Closed Principle)           | 개방/폐쇄 원칙이다.       |
| L    | LSP (Liskov Substitution Principle)   | 리스코프 치환 원칙이다.   |
| I    | ISP (Interface Segregation Principle) | 인터페이스 분리 원칙이다. |
| D    | DIP (Dependency Inversion Principle)  | 의존 역전 원칙이다.       |
